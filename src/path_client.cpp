...
int main(int argc, char **argv) {
    ...
    */
    ...
    pose_stamped.pose.position.x = 0; // say desired x-coord is 0.0
    pose_stamped.pose.position.y = 3; // say desired y-coord is 3.0
    path_srv.request.nav_path.poses.push_back(pose_stamped); //move right again

    pose_stamped.pose.position.x = 7; // say desired x-coord is 7.0
    pose_stamped.pose.position.y = 3; // say desired y-coord is 3.0
    path_srv.request.nav_path.poses.push_back(pose_stamped); //move right again
    
    pose_stamped.pose.position.x = 7; // say desired x-coord is 7.0
    pose_stamped.pose.position.y = 5.3; // say desired y-coord is 5.3
    path_srv.request.nav_path.poses.push_back(pose_stamped); //move right again

    pose_stamped.pose.position.x = 2.8; // say desired x-coord is 2.8
    pose_stamped.pose.position.y = 5.3; // say desired y-coord is 5.3
    path_srv.request.nav_path.poses.push_back(pose_stamped); //move right again

    pose_stamped.pose.position.x = 2.8; // say desired x-coord is 2.8
    pose_stamped.pose.position.y = 12.2; // say desired y-coord is 12.2
    path_srv.request.nav_path.poses.push_back(pose_stamped); //move right again
