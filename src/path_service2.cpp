...
bool callback(example_ros_service::PathSrvRequest& request, example_ros_service::PathSrvResponse& response)
{
    ...
    for (int i=0;i<npts;i++) { //visit each subgoal
        ...
        g_current_pose.position = pose_desired.position; // assumes got to desired position precisely
        ...
        do_move(travel_distance);  // move forward travel distance, compute this from subgoal pose
        }
